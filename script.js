...
function buildBottomNav(active){
  const items=[{href:'index.html',icon:'🏠',label:'หน้าแรก',key:'home'},{href:'category.html',icon:'🍽️',label:'หมวดหมู่',key:'cats'},{href:'search.html',icon:'🔍',label:'ค้นหา',key:'search'},{href:'add-recipe.html',icon:'➕',label:'เพิ่มเมนู',key:'add'},{href:'favorites.html',icon:'❤️',label:'โปรด',key:'favs'},{href:'profile.html',icon:'👤',label:'โปรไฟล์',key:'profile'},{href:'about.html',icon:'ℹ️',label:'เกี่ยวกับ',key:'about'}];
...
function renderSearch(){
  ...
  document.body.innerHTML=`
    ...
    <div class="container"><div class="hero">
      <div class="panel">
        <div class="h1">🔍 ค้นหาเมนู</div>
        <p class="sub">พิมพ์ชื่อเมนูหรือวัตถุดิบที่ต้องการ</p>
        <div class="search-row" style="margin-top:12px">
          <div class="input"><span>🔍</span><input id="searchInput" type="search" placeholder="เช่น ข้าวผัด, ไก่, ผัก..." autofocus></div>
          <button class="btn btn-primary" id="searchBtn">ค้นหา</button>
        </div>
      </div>
      <div class="section">
        <div class="recipes" id="searchResults">
          <div class="empty-state"><div class="empty-icon">🍽️</div><p>พิมพ์เพื่อเริ่มค้นหาเมนู</p></div>
        </div>
      </div>
    </div></div>
    ${buildBottomNav('search')}
  `;
  bindMobileMenu();
  ...
  function doSearch(){
    ...
    if(!q){list.innerHTML='<div class="empty-state"><div class="empty-icon">🍽️</div><p>พิมพ์เพื่อเริ่มค้นหาเมนู</p></div>';return;}
    const results=recipes.filter(r=>
      (r.title||'').toLowerCase().includes(q)||
      (r.ingredients||[]).some(i=>(i.name||'').toLowerCase().includes(q))
    );
    if(!results.length){list.innerHTML='<div class="empty-state"><div class="empty-icon">🤔</div><p>ไม่พบเมนูที่ตรงกับ "'+q+'"</p></div>';return;}
    list.innerHTML=results.map(recipeCard).join('');
    bindFavButtons();
  }
...
function renderProfile(){
  const recipes=getRecipes();
  const favIds=getFavorites();
  const myRecipes=recipes.filter(r=>!r.id.startsWith('seed_'));
  const totalRatings=recipes.reduce((sum,r)=>{const rs=loadJson(STORAGE.ratings,{});return sum+(rs[r.id]?.length||0);},0);
  ...
  document.body.innerHTML=`
    ...
    <div class="container"><div class="hero">
      <div class="panel" style="text-align:center;margin-bottom:14px">
        ...
        <div style="font-size:20px;font-weight:900;margin-bottom:4px">ผู้ใช้งาน</div>
        <p style="font-size:13px;color:var(--muted)">บัญชีในเครื่อง</p>
      </div>
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:14px">
        <div class="card" style="text-align:center;padding:14px 8px">
          <div style="font-size:28px;font-weight:900;color:var(--accent)">${myRecipes.length}</div>
          <div style="font-size:12px;color:var(--muted);font-weight:700">สูตรของฉัน</div>
        </div>
        <div class="card" style="text-align:center;padding:14px 8px">
          <div style="font-size:28px;font-weight:900;color:var(--accent)">${favIds.length}</div>
          <div style="font-size:12px;color:var(--muted);font-weight:700">เมนูโปรด</div>
        </div>
        <div class="card" style="text-align:center;padding:14px 8px">
          <div style="font-size:28px;font-weight:900;color:var(--accent)">${totalRatings}</div>
          <div style="font-size:12px;color:var(--muted);font-weight:700">รีวิวทั้งหมด</div>
        </div>
      </div>
      <div class="card" style="margin-bottom:14px">
        <div style="font-size:16px;font-weight:900;margin-bottom:12px">⚡ ลิงก์ด่วน</div>
        <div style="display:flex;flex-direction:column;gap:8px">
          <a href="add-recipe.html" class="btn" style="justify-content:flex-start;gap:10px">➕ เพิ่มสูตรอาหารใหม่</a>
          <a href="favorites.html" class="btn" style="justify-content:flex-start;gap:10px">❤️ ดูเมนูโปรดทั้งหมด</a>
          <a href="about.html" class="btn" style="justify-content:flex-start;gap:10px">ℹ️ เกี่ยวกับแอพ</a>
        </div>
      </div>
      ${myRecipes.length?`
      <div class="card">
        <div style="font-size:16px;font-weight:900;margin-bottom:12px">📖 สูตรที่ฉันเพิ่ม</div>
        <div class="recipes">${myRecipes.map(recipeCard).join('')}</div>
      </div>`:''}
    </div></div>
...
async function route(){
  ...
  if(path==='search.html'){renderSearch();return;}
  if(path==='profile.html'){renderProfile();return;}
